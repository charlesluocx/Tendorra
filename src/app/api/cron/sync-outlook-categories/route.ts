...
// Unlike sync-gmail, this never reads a whole inbox: listMessagesByCategory
// filters server-side on the Outlook category matching each project's
// email_code, so mail a staff member hasn't tagged is never fetched at all.
export async function POST(request: Request) {
  ...
  if (!connections || connections.length === 0) {
    return NextResponse.json({ connectionsSynced: 0, messagesProcessed: 0 });
  }
  ...
  return NextResponse.json({ connectionsSynced: connections.length, messagesProcessed });
}
