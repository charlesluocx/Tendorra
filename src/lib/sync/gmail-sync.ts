...
// Shared by /api/cron/sync-gmail (every company, on a schedule) and the
// "Sync now" button in settings (one company, on demand) — same dedup and
// matching logic either way.
export async function runGmailSync(companyId?: string) {
  ...
  if (!inboxes || inboxes.length === 0) {
    return { companiesSynced: 0, messagesProcessed: 0 };
  }
  ...
  return { companiesSynced: inboxes.length, messagesProcessed };
}
