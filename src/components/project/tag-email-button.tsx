...
const initialState: TagEmailState = { error: null, notice: null };
...
export function TagEmailButton(props: {
  ...
  return (
    <form action={action} className="shrink-0 text-right">
      ...
      <input type="hidden" name="internet_message_id" value={props.internetMessageId} />
      ...
      {state.notice && <p className="mt-1 max-w-[200px] text-xs text-muted-foreground">{state.notice}</p>}
    </form>
  );
}
